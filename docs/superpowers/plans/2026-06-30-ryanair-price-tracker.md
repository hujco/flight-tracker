...
**Tech Stack:** Python 3.11+, `requests` (HTTP), `sqlite3` (stdlib), `plotly` (grafy), `pytest` (testy), `launchd` (scheduler na macOS).

## Global Constraints

- Python 3.11+ (využíva `datetime.isoformat(timespec=...)`, moderný typing).
...
- [ ] **Step 1: Vytvor `requirements.txt`**
...
Run: `cd ~/www/flight-tracker && python -m pytest tests/test_db.py -v`
...
Run: `cd ~/www/flight-tracker && python -m pytest tests/test_fetch.py -v`
...
Run: `cd ~/www/flight-tracker && python -m pytest tests/test_collect.py -v`
...
Run: `cd ~/www/flight-tracker && python -m pytest tests/test_stats.py -v`
...
Run: `cd ~/www/flight-tracker && python -m pytest tests/test_report.py -v`
...
- [ ] **Step 2: Manuálne over runner (jeden ostrý beh)**
...
> Pozn.: ak `python3` v plist nemá nainštalované `requests`/`plotly`, použi plnú cestu k tomu istému interpreteru, kde si robil `pip install` (zisti cez `which python3`), prípadne k venv `python`.
...
Run: `cd ~/www/flight-tracker && python -m pytest -v`
